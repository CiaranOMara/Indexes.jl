...
function overlapchunks(tabix::Tabix, interval::Interval)
    seqid = findfirst(isequal(seqname(interval)), tabix.names)
    if seqid == 0
        throw(ArgumentError("failed to find sequence name '$(seqname(interval))'"))
    end
    return overlapchunks(tabix.index, seqid, leftposition(interval):rightposition(interval))
end
