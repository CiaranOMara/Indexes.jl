...
function icmp(record, interval)
    c = cmp(BioCore.seqname(record), seqname(interval))

    if c < 0 || (c == 0 && BioCore.rightposition(record) < leftposition(interval))
        return -1
    end

    if c > 0 || (c == 0 && BioCore.leftposition(record) > rightposition(interval))
        return +1
    end
